...
{
	T2CONbits.TON = 0;//定时器关闭
	T2CON = 0x0000;//空闲工作 预分频1:1 不用外部时钟 FOSC/4
//	T2CONbits.TSYNC = 1;//外部时钟
	IEC0bits.T2IE = 1;//允许Timer2中断请求
	INTCON1 |= 0X8000;//不要中断嵌套
	IPC1bits.T2IP = 0x07;//中断优先级7
	T2CONbits.TON = 1;//定时器使能
}
int main()
{
    unsigned int i;
...
void __attribute__((__interrupt__, auto_psv)) _T2Interrupt(void)//定时器1中断
{
